...
import { ArticleAMPQuery } from '../generated/graphql-types';

const ArticleAMP: React.FC<{ data: ArticleAMPQuery }> = ({ data }) => {
  const schema = {
  ...
  const headingImageSrc = getSrc(data.article.frontmatter.image as any);
  const headingImageSrcSet = getSrcSet(data.article.frontmatter.image as any);
  return (
    <Layout>
      <SEO
