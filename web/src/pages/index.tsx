...
export default ({ data: { allSanityHomepage } }) => {
  const [loc, setLoc] = useState('no');
  useEffect(() => {
    setLoc(window.location.href.split('/')[3]);
  }, []);

  const item = allSanityHomepage.nodes[allSanityHomepage.nodes.length - 1];
  if (item && loc)
    return (
      <Layout>
        <Box>
          ...
          <Container sx={{ pt: 6 }}>
            ...
            <Box sx={{ width: '60%' }}>
              {item.homePageTopDescription[`${loc}Text`]
                .map((par) => par.children[0].text)
                .join('\n')}
            </Box>
            <Grid
          ...
          <Container>
            <Box sx={{ fontSize: 'clamp(16px, 8vw, 38px)' }}>
              <Label sx={{ fontWeight: 'normal' }}>
                {item.homePageMiddleTitle[`${loc}Text`]
                  .map((par) => par.children[0].text)
                  .join('\n')}
              </Label>
            </Box>
